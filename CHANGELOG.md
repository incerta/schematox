...
- FEAT: added `.convert(fn)`, a chain method for custom per-field coercion beyond the built-in table (trimming, locale-specific formats, a `"$"`-prefixed currency string, splitting a delimited string into an array, etc.). Like every other struct param, it never touches the schema — `fn` is tracked separately by its position in the _schema_ tree (object key, tuple/union member index, or a `COERCER_PATH_ITEM` sentinel for array/record's one uniformly-applied child position) and fed into `parse()`'s existing `customCoercers` option once composed into a struct via `object()`/`array()`/`record()`/`tuple()`/`union()`. Also like every other struct param, it only applies once — calling it a second time is a compile error, not a silent overwrite, the same as `.brand()`/`.min()`/etc. Independent of `{ coerce: true }`: a declared `.convert()` always runs, the same as `.brand()`/`.min()` need no separate flag to take effect, while `{ coerce: true }` remains its own, separate opt-in for the built-in table — when both apply to the same position, the custom one runs first and the built-in one still runs afterward on its result if coercion was also requested for that call. Raw `parse()` + `customCoercers` (for a static schema with no struct) is independent of `coerce` the same way. ([#83](https://github.com/incerta/schematox/pull/83))
