...
- FEAT: added `withCoercer(struct, fn)` for custom per-field coercion beyond the built-in table (trimming, locale-specific formats, a `"$"`-prefixed currency string, splitting a delimited string into an array, etc.). Also not a schema field — `fn` is tracked by its position in the _schema_ tree (object key, tuple/union member index, or a `COERCER_PATH_ITEM` sentinel for array/record's one uniformly-applied child position) and fed into `parse()`'s existing `customCoercers` option once composed into a struct via `object()`/`array()`/`record()`/`tuple()`/`union()`. Same `{ coerce: true }` gate as the built-in table; a custom coercer runs first and the built-in conversion still runs afterward on its result, so the two compose. Deliberately a free function rather than a `.coercer()` chain method, so it adds no new key to any struct's public shape — this repo's own test suite pins that shape exhaustively per schema type (`tests/by-struct/*`), and a chain method would mean editing all of it for an otherwise self-contained addition. ([#83](https://github.com/incerta/schematox/pull/83))
